...
def normalize_report(raw: Any, source: str = "upload") -> dict[str, Any]:
    ...
    if isinstance(history, list):
        for i, p in enumerate(history):
            if isinstance(p, dict):
                segment = p.get("segment") if isinstance(p.get("segment"), dict) else {}
                pilots.append({
                    "pilot": _text(p.get("pilot")) or f"pilot_{i + 1}", "target_tariff": _text(p.get("target_tariff")),
                    "segment": _text(segment.get("arpu", p.get("filter_arpu_segment", p.get("segment_name")))),
    ...
    return {
        ...
        "source_kind": source if source in SOURCE_LABELS else "upload",
        ...
        "agent_commit": _text(raw.get("agent_commit")),
...
def channel_label(channel: str | None) -> str:
    """Human label for display; raw identifier remains untouched in report/export."""
    return CHANNEL_LABELS.get(channel, channel or "Не указан")


def campaign_rows(report: dict[str, Any], channels: list[str] | None = None) -> list[dict[str, Any]]:
    """Filter campaigns and rank known expected effects high-to-low; unknowns last."""
...
def confidence_label(campaign: dict[str, Any]) -> str:
    """Compact interval-based UI label; it is not a probability of success."""
    low, high = campaign.get("ci95", [None, None])
    if low is None or high is None:
        return "Нет данных"
    if high < 0 or low <= 0 <= high:
        return "Низкая"
    return "Высокая" if high - low <= 0.10 else "Средняя"


def selected_campaign(rows: list[dict[str, Any]], selected_indices: list[int]) -> dict[str, Any] | None:
    """Resolve selection against the filtered rows, never against the original list."""
    if not rows:
        return None
    index = selected_indices[0] if selected_indices else 0
    return rows[index] if 0 <= index < len(rows) else rows[0]


def spend_breakdown(report: dict[str, Any]) -> dict[str, float | int | None]:
    """Separate final-plan and pilot resources without inventing absent totals."""
    final_costs = [c.get("cost") for c in report.get("campaigns", [])]
    final_contacts = [c.get("contacts") for c in report.get("campaigns", [])]
    final_cost = sum(x for x in final_costs if x is not None) if any(x is not None for x in final_costs) else None
    final_reach = sum(x for x in final_contacts if x is not None) if any(x is not None for x in final_contacts) else None
    exploration = report.get("exploration", {})
    return {"final_cost": final_cost, "final_contacts": final_reach,
            "pilot_cost": exploration.get("pilot_cost"), "pilot_contacts": exploration.get("pilot_contacts"),
            "total_cost": report.get("metrics", {}).get("cost"), "total_contacts": report.get("metrics", {}).get("contacts")}
...
def export_csv(report: dict[str, Any]) -> bytes:
    ...
    for c in report["campaigns"]:
        writer.writerow({"source": report["source"], "seed": report.get("seed"), "agent_commit": report.get("agent_commit"),
            "generated_at": report.get("generated_at"), "campaign": c["name"],
...
def export_markdown(report: dict[str, Any]) -> bytes:
    ...
    lines = ["# Beeline Campaign AI — краткое резюме", "", f"- Источник: **{report['source_label']}**",
             ...
             f"- Agent commit: **{report.get('agent_commit') or 'не указан'}**",
             f"- Сформирован: **{report.get('generated_at') or 'не указано'}**",
